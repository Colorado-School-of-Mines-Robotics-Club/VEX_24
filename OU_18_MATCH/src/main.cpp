...
void autonomous() {
	// ParkingStick.tare_position();
	uint32_t timeSinceAutonStart = pros::millis();
	...
	IntakeSpin = 127;
	DT.Drive_IN(55, 50);
	DT.Turn_DEG(50, 50, Direction::LEFT);
	DT.Drive_IN(16, 50);
	IntakeSpin = 0;//temp
	DT.Drive_IN(-2, 50);
	DT.Turn_DEG(92, 50, Direction::LEFT);
	//DT.Drive_IN(2, 50);
	//IntakeSpin = 0;
	//DT.Drive_IN(-2, 50);
	ArmRotate.move_absolute(1200, 200);
	pros::delay(500);
	DT.Drive_TIME(1500, -30);
	ArmRotate.brake();
	IntakeSpin = 127;
	ArmExtend.move_absolute(1150, 100);
	pros::delay(500);
	int timeout = 150;
	while (timeout > 0 && (IntakeSpin.get_actual_velocities()[0] > 0 || IntakeSpin.get_actual_velocities()[1] > 0))
	{
	timeout--;
	pros::delay(10);
	}
	pros::delay(500);
	ArmExtend.move_absolute(0, 200);
	ArmRotate.move_absolute(0, 200);
	pros::delay(1000);
	DT.Drive_IN(1, 50);//formerly 3
	DT.Turn_DEG(78, 50, Direction::RIGHT);//formerly 75
	ArmRotate.brake();
	Wings.set_value(true);
	pros::delay(500);
	Wings.set_value(false);
	IntakeSpin = -127;//changed was 0
	DT.Drive_TIME(1500, 127);

	double dist;//if >290 go back less;
	double dist2;
	if (true){//DT.getHeading2() > 290) {
		dist = 12;
		dist2 = 33;
	} else {
		dist = 22;
	...
	DT.Drive_IN(-dist, 50);
	IntakeSpin = 0;
	DT.Turn_RHD(18, 30);//used to be 16, didnt worky
	ArmRotate.move_absolute(1500, 200);
	DT.Drive_IN(-dist2, 50);
	ArmRotate.brake();
	IntakeSpin = 127;
	ArmExtend.move_absolute(1150, 200);
	pros::delay(1500);
	timeout = 150;
	while (timeout > 0 && (IntakeSpin.get_actual_velocities()[0] > 0 || IntakeSpin.get_actual_velocities()[1] > 0))
	{
	...
	ArmExtend.move_absolute(0, 200);
	ArmRotate.move_absolute(0, 200);
	DT.Drive_TIME(2500, -33);
	DT.Strafe_TIME(2000, 70, Direction::RIGHT);
	DT.Drive_TIME(1000, -33);

	ArmRotate.brake();
	ArmExtend.brake();

	while (pros::millis() - timeSinceAutonStart < 30000){

	}

	DT.Strafe_IN(37, 60, Direction::LEFT);
	Wings.set_value(true);
	IntakeSpin = 0;
	pros::delay(500);
	...
	Wings.set_value(false);
	pros::delay(500);
	DT.Turn_DEG(20, 50, Direction::LEFT);//was 35
	DT.Drive_IN(-17, 50);
	DT.Turn_DEG(85, 60, Direction::LEFT);
	ParkingStick.move_absolute(-450, 200);
	DT.Strafe_TIME(2000, 127, Direction::LEFT);
	DT.Drive_TIME(1000, -50);
...
void opcontrol() {
	...
	while (true) {
		...
		if(pros::millis() - timeSinceStart > 500){
			ParkingStick[0].set_current_limit(0);
			ParkingStick[1].set_current_limit(0);
			ParkingStick = 0;
		}


		// if (ControllerMain.get_digital(pros::E_CONTROLLER_DIGITAL_X)){
		// 	DT.setCurrentLimit(0);
		// } else if (ControllerMain.get_digital(pros::E_CONTROLLER_DIGITAL_B)){
		// 	DT.setCurrentLimit(2500);
		// }
